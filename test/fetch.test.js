...
describe('Fetch service', function() {
    describe('runApi function', function() {
        ...
        it('should return an array with the data of the matches', async () => {
            ...
            result[0].should.include.keys('team1', 'team2', 'score',
                                          'currentStatus');
        });
    })
    ...
    describe('configMessage function', function() {
        it('should return the correct string in Portuguese', function() {
            ...
            assert.equal(JSON.parse(fetch.configMessage(game)).title,
                         'Gol do Atletico Mineiro!');
            assert.equal(JSON.parse(fetch.configMessage(game)).body,
                         'Atletico Mineiro 3-1 Cruzeiro');
            assert.equal(JSON.parse(fetch.configMessage(game)).icon,
                         'images/ball.png');
        })
    })
    ...
    describe('checkGameStatus function', function() {
        it('should return a string if there is a new status in the game and null otherwise',
           function() {
               ...
               assert.equal(fetch.checkGameStatus(),
                            null); // invalid entry data
